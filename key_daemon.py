...
def start_recording():
    ...
    recording_stream = sd.InputStream(samplerate=NATIVE_RATE, channels=CHANNELS, callback=audio_callback, dtype='int16')
...
try:
    for event in dev.read_loop():
        if event.type == ecodes.EV_KEY:
            ...
            keycode = data.keycode[0] if isinstance(data.keycode, list) else data.keycode
